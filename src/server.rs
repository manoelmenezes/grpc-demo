...
impl Inventory for StoreInventory {
    async fn add(
        ...
        // validate SKU, verify that it is present and not empty
        ...
        // if the item is already present do not allow the duplicate
    ...
    async fn update_quantity(
        ...
        // quantity changes with no actual change do not make sense, inform user
        if change.change == 0 {
    ...
    async fn update_price(
        ...
        // $0.00 disallowed and negatives do not make sense, inform the user
        if change.price <= 0.0 {
            return Err(Status::invalid_argument(BAD_PRICE_ERR));
        }
    ...
    async fn watch(
        ...
        // the channel will be our stream back to the client, we will send copies
        // of the requested item any time we notice a change to it in the
        // inventory.
        let (tx, rx) = mpsc::unbounded_channel();

        // we will loop and poll new copies of the item until either the client
        // closes the connection, or an error occurs.
        let inventory = self.inventory.clone();
        tokio::spawn(async move {
            loop {
                // it is somewhat basic, but for this demo we will just check the
